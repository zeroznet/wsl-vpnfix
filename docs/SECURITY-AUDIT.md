...
- F-014 gvproxy -listen-stdio CLI flag never wired into config — upstream gvisor-tap-vsock v0.8.8 `cmd/gvproxy/config.go:125` parses `-listen-stdio` but never assigns it into `config.Interfaces.Stdio`, so the stdio acceptor goroutine never starts and the bridge is TX-only; workaround stages a YAML config that sets `interfaces.stdio` directly, bypassing the broken CLI path; status: fixed-in-v0.1.0 (`e30d045`), re-evaluate at next gvisor-tap-vsock release.

## Known gaps

- F-015 default-route persistence to disk for crash-safe recovery — orchestrator deletes the original WSL2 default route at init; a mid-init crash leaves sibling distros without networking until `wsl --shutdown` because the original routes are held only in process memory; status: tracked-in-TODO (added in Phase C C3 PR).
- F-016 no wsl-vpnfixctl debug subcommand — no built-in `status`, `dump-config`, or `verify-pins` command for operator diagnostics; failure diagnosis requires manual `nft list table wsl-vpnfix` + `ip route` + log inspection; status: tracked-in-TODO (added in Phase C C3 PR).

## Tracked workarounds

- F-017 govulncheck non-blocking in CI — Go 1.25.9 carries 2 stdlib CVEs (`GO-2026-4971` net/Dial NUL injection on Windows, `GO-2026-4918` net/http HTTP/2 SETTINGS_MAX_FRAME_SIZE infinite loop) fixed in Go 1.25.10; alpine apk pin blocks the bump until Alpine ships the updated package; both are low exposure for this threat model (Linux-only target, gvproxy-trusted HTTP/2 peer); CI step runs with `continue-on-error: true`; status: tracked-in-TODO (Backlog), re-enable strict govulncheck when alpine apk ships Go 1.25.10.
- F-018 gvproxy v0.8.8 stdio regression tracked vs upstream — the `-listen-stdio` wiring defect in `cmd/gvproxy/config.go:125` (F-014) is a known upstream regression, not a local fix; the `-config` YAML workaround is in production; status: fixed-in-v0.1.0 (`e30d045`), re-evaluate at next gvisor-tap-vsock release.
