...
- F-018 gvproxy.exe staged into a world-writable Windows directory — the 9P paging workaround (F-010) stages `wsl-gvproxy.exe` into `C:\Users\Public\.wsl-vpnfix\`, which inherits an `Authenticated Users: Modify` NTFS ACL, so any other local Windows user can replace the binary after the stage-time sha256 check (gvforwarder re-execs it from that path on every retry with no re-verification); on single-user machines (the primary deployment) the boundary is moot, on shared corporate machines it is a real cross-user tampering surface; DrvFs cannot set NTFS ACLs and the no-shelling-out rule precludes `icacls` from the orchestrator, so a fix needs installer-side pre-staging into `%LOCALAPPDATA%`; status: accepted-for-now, mitigation candidate tracked with the installer work.
...
- F-017 gvproxy v0.8.8 stdio regression — upstream `cmd/gvproxy/config.go:125` parses `-listen-stdio` but never assigns it into `config.Interfaces.Stdio`; the stdio acceptor goroutine never starts, leaving the bridge TX-only; without the workaround, gvproxy.exe has no path back to gvforwarder and the whole bridge is silently dead; workaround stages a YAML config that sets `interfaces.stdio` directly, bypassing the broken CLI path; status: fixed-in-v0.1.0 (`e30d045`); upstream fixed the CLI wiring in v0.8.9 ("fix(gvproxy): propagate --listen-stdio flag into config", verified in source: `args.stdioSocket` → `config.Interfaces.Stdio`), pin bumped 2026-07-29; workaround removed 2026-07-30 — the stdio URL is back to `listen-stdio=accept` with `ssh-port=-1` re-added (leaving `-config` mode re-enables default mode's 127.0.0.1:2222 SSH listener — F-011).
