...
|---|---|
| 2.6 Rootfs contents | Drop `/etc/systemd/system/wsl-vpnfix.service`. Drop the `wsl.conf` `[boot] systemd=true` flag. Add `/sbin/init` symlink → `/sbin/wsl-vpnfix`. Final rootfs: `/sbin/wsl-vpnfix`, `/sbin/wsl-gvforwarder`, `/etc/wsl-vpnfix/wsl-gvproxy.exe`, `/etc/wsl-vpnfix/checksums`, `/etc/wsl.conf`, `/sbin/init` symlink, `LICENSE`, plus busybox, Alpine baselayout, `nftables`, `iproute2`, `ca-certificates`. No `bash`, no `curl`, no `wget`, no compilers, no SSH. |
