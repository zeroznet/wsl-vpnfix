...
|---|---|---|---|
| D-1 | Init mechanism inside the appliance distro | **PID 1 (the Go orchestrator runs as `init` directly)** | Appliance runs exactly one service. Systemd unit-hardening (`ProtectSystem`, `PrivateTmp`, `NoNewPrivileges`, `CapabilityBoundingSet`) exists to scope processes from each other in a multi-service host; with one process there is nothing to scope from. Runtime needs `CAP_NET_ADMIN` for the lifetime of the appliance, so privilege drop is impossible — the "minimum privilege" hardening story does not apply. PID-1 reaper plus signal forwarder is a well-known ~50-line Go pattern (cf. `tini`, `dumb-init`). Drops ~10 MB of systemd + dbus + journald from the rootfs and the corresponding CVE / audit surface. |
...
|---|---|
| 2.6 Rootfs contents | Drop `/etc/systemd/system/wsl-vpnfix.service`. **Keep** the `wsl.conf` `[boot] systemd=true` flag — it is a Microsoft-named switch that makes WSL `/init` exec `/sbin/init` as PID 1 in a PID namespace; it is NOT a dependency on systemd the project. Our rootfs still ships zero systemd binaries (no `apk add systemd`, no dbus, no journald). Add `/sbin/init` symlink → `/sbin/wsl-vpnfix`. Final rootfs: `/sbin/wsl-vpnfix`, `/sbin/wsl-gvforwarder`, `/etc/wsl-vpnfix/wsl-gvproxy.exe`, `/etc/wsl-vpnfix/checksums`, `/etc/wsl.conf` (with `systemd=true`), `/sbin/init` symlink, `LICENSE`, plus busybox, Alpine baselayout, `nftables`, `iproute2`, `ca-certificates`. No `bash`, no `curl`, no `wget`, no compilers, no SSH. |
